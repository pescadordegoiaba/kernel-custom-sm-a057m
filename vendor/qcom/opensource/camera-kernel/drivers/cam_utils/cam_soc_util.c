...
static unsigned int cam_perf_mode;

#define CAM_PERF_MODE_STOCK	0
#define CAM_PERF_MODE_LATENCY	1
#define CAM_PERF_MODE_POWER	2

static int cam_perf_mode_set(const char *val, const struct kernel_param *kp)
{
	unsigned int new_mode;
	int rc;

	rc = kstrtouint(val, 0, &new_mode);
	if (rc)
		return rc;
	if (new_mode > CAM_PERF_MODE_POWER)
		return -EINVAL;

	return param_set_uint(val, kp);
}

static const struct kernel_param_ops cam_perf_mode_ops = {
	.set = cam_perf_mode_set,
	.get = param_get_uint,
};

module_param_cb(cam_perf_mode, &cam_perf_mode_ops, &cam_perf_mode, 0644);
MODULE_PARM_DESC(cam_perf_mode,
	"Camera clock vote policy: 0=stock, 1=latency, 2=power");
...
static enum cam_vote_level cam_soc_util_find_valid_level_at_least(
	struct cam_hw_soc_info *soc_info, enum cam_vote_level min_level)
{
	enum cam_vote_level level;

	for (level = min_level; level < CAM_MAX_VOTE; level++)
		if (soc_info->clk_level_valid[level])
			return level;

	return CAM_MAX_VOTE;
}

static enum cam_vote_level cam_soc_util_find_valid_level_at_most(
	struct cam_hw_soc_info *soc_info, enum cam_vote_level max_level)
{
	int level;

	for (level = max_level; level > CAM_SUSPEND_VOTE; level--)
		if (soc_info->clk_level_valid[level])
			return level;
...
static enum cam_vote_level cam_soc_util_apply_perf_mode(
	struct cam_hw_soc_info *soc_info, enum cam_vote_level level)
{
	enum cam_vote_level target;

	if (!soc_info || level <= CAM_SUSPEND_VOTE || level >= CAM_MAX_VOTE)
		return level;

	switch (cam_perf_mode) {
	case CAM_PERF_MODE_LATENCY:
		if (level >= CAM_NOMINAL_VOTE)
			return level;
		target = cam_soc_util_find_valid_level_at_least(soc_info,
			CAM_NOMINAL_VOTE);
		return (target < CAM_MAX_VOTE) ? target : level;
	case CAM_PERF_MODE_POWER:
		if (level <= CAM_SVSL1_VOTE)
			return level;
		target = cam_soc_util_find_valid_level_at_most(soc_info,
			CAM_SVSL1_VOTE);
		return (target < CAM_MAX_VOTE) ? target : level;
	default:
		return level;
	...
	int64_t clk_rate)
{
	...
	if (!clk_level_override) {
		apply_level = cam_soc_util_apply_perf_mode(soc_info,
			apply_level);
		clk_rate = soc_info->clk_rate[apply_level][src_clk_idx];
	}
	...
	enum cam_vote_level clk_level)
{
	...
	if (rc)
		return rc;
	if (!soc_info->clk_level_override)
		apply_level = cam_soc_util_apply_perf_mode(soc_info,
			apply_level);
