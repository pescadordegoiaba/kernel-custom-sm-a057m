...
/*
 * Maximum number of CPUs this cooling driver may keep offline at once.
 * The thermal cooling devices themselves remain binary (online/offline).
 */
static unsigned int cpu_hotplug_level = 1;
...
static unsigned int cpu_hotplug_offline_count_locked(void)
{
...
static void cpu_hotplug_queue_all_locked(void)
{
	struct cpu_hot_cdev *cpu_hot_cdev;
...
static int cpu_hotplug_level_set(const char *val, const struct kernel_param *kp)
{
	unsigned int new_level;
	int ret;

	ret = kstrtouint(val, 10, &new_level);
	if (ret)
		return ret;
	if (new_level > 3)
		return -EINVAL;
	...
	*(unsigned int *)kp->arg = new_level;
	cpu_hotplug_queue_all_locked();
	mutex_unlock(&cpu_hot_lock);

	return 0;
}

static const struct kernel_param_ops cpu_hotplug_level_ops = {
	.set = cpu_hotplug_level_set,
	.get = param_get_uint,
};

module_param_cb(cpu_hotplug_level, &cpu_hotplug_level_ops,
		&cpu_hotplug_level, 0644);
MODULE_PARM_DESC(cpu_hotplug_level,
		 "Maximum CPUs kept offline by thermal hotplug (0-3). "
		 "0 disables only this cooling device; 1=conservative "
		 "(recommended), 2=moderate, 3=aggressive. Other thermal "
...
static int cpu_hot_hp_online(unsigned int online_cpu)
{
	...
	list_for_each_entry(cpu_hot_cdev, &cpu_hot_cdev_list, node) {
		...
		if (cpu_hot_cdev->cdev) {
			offline_count = cpu_hotplug_offline_count_locked();
			if (cpu_hot_cdev->cpu_hot_state &&
			    cpu_hotplug_level > 0 &&
			    offline_count <= cpu_hotplug_level) {
				 ...
				 unsigned long state)
{
	...
	/* Each cooling device is binary; the module parameter caps the total. */
	if (state > 1)
		return -EINVAL;
...
static void cpu_hot_execute_cdev(struct work_struct *work)
{
	...
	offline_count = cpu_hotplug_offline_count_locked();
	should_offline = cpu_hot_cdev->cpu_hot_state &&
			cpu_hotplug_level > 0 &&
			((cpu_hot_cdev->cpu_cur_state &&
			  offline_count < cpu_hotplug_level) ||
			 (!cpu_hot_cdev->cpu_cur_state &&
			  offline_count <= cpu_hotplug_level));
