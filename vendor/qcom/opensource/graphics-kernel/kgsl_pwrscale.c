...
#define KGSL_GOV_INTERVAL_MIN_US	5000
#define KGSL_GOV_INTERVAL_MAX_US	50000

static unsigned int governor_call_interval_us = 10000;

static int governor_call_interval_us_set(const char *val,
		const struct kernel_param *kp)
{
	...
	if (ret)
		return ret;
	if (new_val < KGSL_GOV_INTERVAL_MIN_US ||
	    new_val > KGSL_GOV_INTERVAL_MAX_US)
		return -EINVAL;
	...
	if (device && device->pwrscale.enabled) {
		mutex_lock(&device->mutex);
		device->pwrscale.next_governor_call = ktime_add_us(ktime_get(),
				governor_call_interval_us);
...
static const struct kernel_param_ops governor_call_interval_us_ops = {
	.set = governor_call_interval_us_set,
	.get = param_get_uint,
};

module_param_cb(governor_call_interval_us, &governor_call_interval_us_ops,
		&governor_call_interval_us, 0644);
MODULE_PARM_DESC(governor_call_interval_us,
		 "devfreq governor minimum call interval in microseconds (5000-50000)");
